...
# 기본 토픽/프레임
# ──────────────────────────────────────────────────────────────────────────────
INPUT_TOPIC = '/fused_sorted_cones_ukf'#'/cone/lidar/ukf'
...
STEP_LEN = 1.5#1.2#1.0#0.8#1.0           # (m) 기본 스텝 길이 (마지막 스텝은 자동 축소)
    ...
    def _get_start_node(self, pts: List[Tuple[float, float]],
        ...
        try:
            cands = [(i, x) for i, (x, y) in enumerate(pts)
                     ...
                     #print(min(cands, key=lambda it: it[1])[0] if cands else None)
